"""Thin entrypoint: ingest GSE184950 (PDD vs control, postmortem SN) into interim h5ad."""
import argparse
import json
import sys
from pathlib import Path

from pdproduct.datasets.tenx import ingest_gse184950
...
def main() -> int:
    ...
    manifest = new_run_manifest(
        ...
        command="python scripts/preprocess_gse184950.py",
    )
    write_run_manifest(manifest, args.run_dir)

    report = ingest_gse184950(Path(args.raw_tar), Path(args.extract_dir), Path(args.out_h5ad), Path(args.meta_json))
    ...
    summary = {k: report[k] for k in (
        "n_cells", "n_genes", "n_samples", "n_donors", "condition_donor_counts",
        "multi_sample_donors", "out_h5ad_bytes",
    ) if k in report}
