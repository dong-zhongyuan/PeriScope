"""Annotate Moquin-Beaudry 2025 PBMC cells: same filter/markers as GSE223138.

Copy+edit of annotate_gse223138.py (pd_product rule). Prereg v2 (f64fc3e): re-annotate
with the SAME PBMC marker set for cross-cohort method identity; author annotations
(predicted.celltype.l1/l2, final_broad) kept in obs for sensitivity. Input X is the
author LogNormalize layer. QC columns n_genes_by_counts/pct_mt from author metadata.
"""
import argparse
import hashlib
import json
...
def main() -> int:
    ...
    manifest = new_run_manifest(
        ...
        command="python scripts/annotate_mb2025.py",
    )
