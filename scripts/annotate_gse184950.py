"""Annotate GSE184950 (PDD cohort): same pre-registered filter + marker scoring
as GSE178265. Conditions are PDD vs Control; ALDH1A1 directional output included.
"""
import argparse
import json
import sys
...
def main() -> int:
    ...
    manifest = new_run_manifest(
        "annotate-gse184950-v1",
        approved_scope="phaseB two-stage replication, stage-2 cohort annotation (ADR-0001)",
        command="python scripts/annotate_gse184950.py",
    )
    ...
    ct_donor = adata.obs.groupby(["donor", "cell_type"], observed=True).size().unstack(fill_value=0)
    cond = adata.obs.groupby("donor", observed=True)["condition"].first()
    da_frac = (ct_donor.get("DA_neuron", 0).T / ct_donor.sum(axis=1)).T
    da_by_cond = {
        c: {"mean_da_fraction": float(da_frac[[d for d in da_frac.index if cond[d] == c]].mean()),
            "n_donors": len([d for d in da_frac.index if cond[d] == c])}
    ...
    print(json.dumps({k: report[k] for k in ("n_cells_before", "n_cells_after", "n_per_type", "da_fraction_by_condition")}, ensure_ascii=False, indent=1))
    return 0
